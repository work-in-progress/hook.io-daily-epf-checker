(function() {
  var DailyEpfCheck, Hook, async, colors, epf, mongoose, pts, util;
  ...
  async = require('async');
  DailyEpfCheck = exports.DailyEpfCheck = function(options) {
    var self;
    self = this;
    Hook.call(self, options);
    return self.on("hook::ready", function() {
      mongoose.connect(self.mongoose.connection);
      self.on("daily-epf-checker::epf-status-received", function(data) {
        return self._epfStatusReceived(data);
      });
      self.on("daily-epf-checker::check-epf-status", function(data) {
        return self._checkEpfStatus();
      });
      self.on("daily-epf-checker::ensure-tasks-finished", function(data) {
        return console.log("ensure task exists finished");
      });
      self.emit("daily-epf-checker::check-epf-status", {});
      return setInterval((__bind(function() {
        return self.emit("daily-epf-checker::check-epf-status", {});
      }, this)), 1000 * 60 * 10);
    });
  };
  util.inherits(DailyEpfCheck, Hook);
  DailyEpfCheck.prototype._checkEpfStatus = function(data) {
    ...
    return epf.check(this.epfserver.auth.username, this.epfserver.auth.password, __bind(function(err, data) {
      if (err) {
        console.error(err);
        return this.emit("daily-epf-checker::epf-status-error", {
          error: err
        });
      } else {
        return this.emit("daily-epf-checker::epf-status-received", {
          checkResult: data
        });
      }
    }, this));
  };
  DailyEpfCheck.prototype._ensureSubTasksExists = function(taskContainer, fullOrPartial, isFull, cb) {
  ...
  DailyEpfCheck.prototype._epfStatusReceived = function(data) {
    ...
    return async.forEachSeries(items, __bind(function(item, cb2) {
      ...
      if (err) {
        console.error(err);
        return this.emit("daily-epf-checker::ensure-tasks-error", {
          error: err
        });
      } else {
        return this.emit("daily-epf-checker::ensure-tasks-finished", {});
      }
    }, this));
  };
}).call(this);
