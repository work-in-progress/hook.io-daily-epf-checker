(function() {
  ...
  DailyEpfImport = exports.DailyEpfImport = function(options) {
    ...
    return self.on("hook::ready", function() {
      ...
      self.on("daily-epf-import::epf-status-received", function(data) {
        return self._epfStatusReceived(data);
      });
      self.on("daily-epf-import::check-epf-status", function(data) {
      ...
      return self.emit("daily-epf-import::check-epf-status", {});
    });
  };
  ...
  DailyEpfImport.prototype._checkEpfStatus = function(data) {
    ...
    return epf.check(this.epfserver.auth.username, this.epfserver.auth.password, __bind(function(err, data) {
      if (err) {
        console.error(err);
        return this.emit("daily-epf-import::epf-status-error", {
          error: err
        });
      } else {
        return this.emit("daily-epf-import::epf-status-received", {
          checkResult: data
        });
      }
    }, this));
