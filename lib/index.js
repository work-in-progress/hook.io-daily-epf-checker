  ...
  DailyEpfImport.prototype._epfStatusReceived = function(data) {
    ...
    return pts.getOrCreateTaskContainer("epf::status-import::" + data.checkResult.full.date.asString, __bind(function(err, taskContainer, isNew) {
      if (err) {
        ...
        return this._ensureSubTasksExists(taskContainer, data.checkResult.full, true, __bind(function(err) {
          if (err) {
            return console.error(err);
          } else {
            return async.forEachSeries(data.checkResult.incremental, __bind(function(inc, cb2) {
              return pts.getOrCreateTaskContainer("epf::status-import::" + inc.date.asString, __bind(function(err, taskContainer2, isNew) {
                if (err != null) {
                  return cb2(err);
                }
                return this._ensureSubTasksExists(taskContainer2, inc, false, cb2);
              }, this));
            }, this), __bind(function(err) {
              if (err) {
                console.error(err);
